...
Dave reports Claude is actively drafting L-202-08 and L-202-09 locally. Reserve those lesson files and their dedicated assets for Claude. Grokbot claimed ENG-004 on `cursor/eng-004-export-preview-42be`. The coordinator owns shared board/handoff updates during this parallel work; workers write their own logs. Login/provider selection remains deferred to DATA-001.
...
| [ ] | ENG-005 | TODO | - | ENG-001 | Add Saving/Saved/Could not save feedback and preserve edits on storage failure. Verify quota/unavailable-storage behavior and recovery. Preserve the REV-001 failed-load write guard. Explain when edits are only in memory; offer recovery without overwriting the unreadable raw copy. |
