...
### 1. Observed defect: a one-on-one cannot reach insight, next step, or prayer without scrolling the whole lesson
...
- **Observed:** Those fields exist only at the bottom of the Read scroll. TOC lists lessons, not sections. There is no jump to the widget, the closing trio, or last week's step.
...
### 2. Observed defect: the completed Grace diagram cannot be seen as a whole
...
- **Observed:** After the fifth Next, measured iframe height was 649px at 375/390 and 683px at 1280. With the sticky lesson topbar (about 85px), the Grace and Mercy labels are clipped. Only the lower descriptions, the Righteous and Justified boxes, the verse card, and Back/Restart remain on screen. The widget title bar is covered. A Saved banner, when present, takes still more vertical space.

Reproduction: open `#201-8`, tap Begin, tap Next until Restart appears, scroll until Back and Restart are in view. The top boxes show "getting a GIFT you don't deserve" and "not getting the PUNISHMENT you do deserve" with their names cut off. The 201-01 completed Great Command at the same 375px alignment keeps title, diagram, card, and controls on screen (iframe 480px).
...
### 4. Observed defect: 201-01 running-Father alt text describes an embrace the picture does not show
...
- **Observed:** The file shows a father running down a road toward a small distant figure. It does not show an embrace. Color is a scroll-reveal: black-and-white opacity was 1.0 when the image first entered view, 0.821 when centered, and 0.197 after another 280px of scroll at 375px. The JSON alt string is "A father lifting his robe and running down the road to embrace his returning son".
...
6. **Prior commitment.** One tap to `question-201-201-07-step` when present, labeled as last week's step. Do not open a different course. If that field is empty, offer 8.3 (`question-201-201-08-3`) as this week's mercy application, collapsed by default so private detail is not the first thing on the screen.
